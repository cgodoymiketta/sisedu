	...
	?>
 	...
 	<tr>
		<td ><table width="100%" border="0">
		  <tr>
    <td width="10%">&nbsp;</td>
    <td width="80%" align="center">REPORTES PARCIALES POR MATERIA<? //=$VLPer; ?></td>
    <td width="10%">&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
<?php
		...
		$VtqueryC= "select * from crs where curcodigo=".$VLdtCamp4;
		$VtqueryP= "select * from prll where parcodigo=".$VLdtCamp19;

		$VtqueryM= "SELECT q.quicodigo, q.quiorden, m.matcodigo, m.matdescripcion, 
		q.quidescripcion, p.prccodigo, p.prcdescripcion, p.prcorden, p.prcversion
FROM qmstrdtll dq, qmstr q, prcl p, mtr m, grpmtr gm, grp g, spcldd e, dcntmtr dm ";
if ( $VLPer==4 || $VLPer==7 ){
	$VtqueryM= $VtqueryM.", grpprllmtrdcnt gpmd  ";
}
$VtqueryM= $VtqueryM." WHERE p.prcestado >1
...
if( $VLPer==7 ){
	$VtqueryIns= " SELECT q.quicodigo, q.quiorden, m.matcodigo, m.matdescripcion, q.quidescripcion,
...
ORDER by q.quiorden desc, p.prcorden desc, p.prcdescripcion, dq.matcodigo ";
	...
	if ($VLPer==7){
	$VLdtDatosI = execute_query($VtqueryIns,$VLConexion);
	$VLnuDatosI = total_records($VLdtDatosI);
	}
...
//////////////////  para el caso del inspector
	...
	if ( $VLnuDatosI>0 )
	{
		...
		{
			$VTCampo1=get_result($VLdtDatosI,$i,"m.matcodigo");
			$VTCampo2=get_result($VLdtDatosI,$i,"m.matdescripcion");
			$VTCampo3=get_result($VLdtDatosI,$i,"q.quicodigo");
			$VTCampo4=get_result($VLdtDatosI,$i,"q.quidescripcion");
			$VTCampo5=get_result($VLdtDatosI,$i,"p.prccodigo");
			$VTCampo6=get_result($VLdtDatosI,$i,"p.prcdescripcion");
			$VTCampo14=get_result($VLdtDatosI,$i,"p.prcversion");
