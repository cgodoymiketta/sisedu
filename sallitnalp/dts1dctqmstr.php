		...
		for ($i=0; $i<$VtNumero; $i++)
		{
			...
			if ( $VLnuDatosQ > 0)
			{
					...
					if ( $VLdtmattipo[$i]== 1 ) 
					{
						...
						}else{////////////////PARA EL RESTO DE LOS AÑOS
					...
					if ( $VLdtmattipo[$i]== 4 ) 
					{
						if ( trim($VTdtPromParcial[$i])==""){$VTdtPromParcial[$i]=0;}
						if ( trim($VTdtExamen[$i])==""){$VTdtExamen[$i]=0;}
						if ( trim($VTdtEquiv20[$i])==""){$VTdtEquiv20[$i]=0;}
...
<table width="100%" border="0">
	<tr>
	  <td ><?php include("mnuccndctqmstr.php"); ?></td>
	</tr>
	  <tr>
		<td ><hr></td>
	  </tr>
	  <tr>
		...
		{
			include("dtslstd2dctqmstr.php"); 
		} else {
		 	include("dtslstd1dctqmstr.php"); 
		 }
		 ?></td>
	</tr>
</table>